...
###   This script attempts to download the signature file SHA256SUMS.asc from fatbit.org
...
WORKINGDIR="/tmp/fatbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://fatbit.org/bin/"
VERSIONPREFIX="fatbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'fatbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Fatbit Core binary release signing key installed?"
   fi
